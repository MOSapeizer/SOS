...
xmlhttp.onreadystatechange=function()
{
  ...
  {
    parseCapabilities(xmlhttp.responseText);
  }
};
...
for(i=0;i<xmlDoc.getElementsByTagName("Operation").length;i++){
  if(xmlDoc.getElementsByTagName("Operation")[i].getAttribute('name')=="GetObservation"){
    for(j=0;j<xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter").length;j++){
      if(xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getAttribute('name')=="offering"){
        for(k=0;k<xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getElementsByTagName("Value").length;k++){
          str += '<option value='+xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getElementsByTagName("Value")[k].childNodes[0].nodeValue+'>'
              + xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getElementsByTagName("Value")[k].childNodes[0].nodeValue+'</option>';
        }
      }
      else if(xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getAttribute('name')=="observedProperty"){
        for(k=0;k<xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getElementsByTagName("Value").length;k++){
          str2 += '<option>'+xmlDoc.getElementsByTagName("Operation")[i].getElementsByTagName("Parameter")[j].getElementsByTagName("Value")[k].childNodes[0].nodeValue+'</option>';
        }
      }
    }
  }
...
document.getElementById("offeringID").innerHTML = str;
document.getElementById("property").innerHTML = str2;
}

function gettime(offering) {
  for(i=0;i<xmlDoc.getElementsByTagName("offering").length;i++){
    if(xmlDoc.getElementsByTagName("offering")[i].getElementsByTagName("identifier")[0].childNodes[0].nodeValue==offering){
      str3 = xmlDoc.getElementsByTagName("offering")[i].getElementsByTagName("beginPosition")[0].childNodes[0].nodeValue;
      str4 = xmlDoc.getElementsByTagName("offering")[i].getElementsByTagName("endPosition")[0].childNodes[0].nodeValue;
    }
  }
  document.getElementById("startTime").value = str3;
  document.getElementById("endTime").value = str4;
}
...
function doGetObservation() {
  ...
  var sosURL = document.getElementById("sosURL").value;
...
if(document.getElementById("sosURL").value=="http://cgis.csrsr.ncu.edu.tw:8080/swcb-sos-new/service"){
  ...
  icon = new TGOS.TGImage("http://i.imgur.com/8Q1v9Ab.png?2",
    new TGOS.TGSize(38, 33), new TGOS.TGPoint(0, 0), new TGOS.TGPoint(10, 33));
}
...
/*
TODO: Modify the <div id="container"></div>'s style to width with 100% and height with 400px. This placeholder will be used to show the time-series chart.
*/

var reading = read`ingArray[foiName];
...
/*
TODO: Here you will need to use the HighChart library to create a time-series chart and use the <div id="container"></div> as the placeholder.
You will need to use the prepared "observationArray" object as the input data for HighChart library.
The style of this time-series chart should be similar to the example figure shown in the Lab 8 document.
You will need to implement the chart requirements mentioned in the Lab 8 document as well.
*/
}